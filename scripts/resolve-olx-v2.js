...
function normalizeComplexName(name) {
  if (!name) return '';
  return name
    .replace(/^(жк|житловий комплекс|жилой комплекс|кг|км|котеджне містечко|коттеджный городок|апарт.комплекс|клубний будинок|клубный дом)\s+/gi, '')
    .replace(/[«»""''`'()]/g, '')
    .replace(/\s+/g, ' ')
    .trim()
    .toLowerCase();
...
async function main() {
  ...
  console.log('=== OLX Resolution v2: Streets + Enriched Complexes ===\n');
  ...
  // ====== LOAD ENRICHED COMPLEXES (only those with street_id) ======
  console.log('Loading enriched complexes...');
  ...
  for (const c of complexesResult.rows) {
    ...
    if (nameRu && nameRu.length >= 3) {
      complexMap.set(nameRu, complex);
    }
    if (nameUk && nameUk.length >= 3 && nameUk !== nameRu) {
      complexMap.set(nameUk, complex);
    }
  }
  console.log(`Loaded ${complexMap.size} unique enriched complex names\n`);
  ...
  const complexPattern = new RegExp(
    `(?:жк|житловий комплекс|жилой комплекс|кг|км|апарт.комплекс)\\s*["«']?(${escapedComplexNames.join('|')})["»']?`,
    'gi'
  );
